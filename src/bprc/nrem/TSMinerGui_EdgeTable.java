...
public class TSMinerGui_EdgeTable extends JPanel implements ActionListener {
	 ...
	 */
	public TSMinerGui_EdgeTable(JDialog dialog, JFrame frame1,
			TSMiner_Timeiohmm theTimeiohmm, TSMiner_Timeiohmm.Treenode ptr,
			int ndepth, int nchild, int ntime, int alltime, boolean broot, int npermutationval) {
		...
		if(alltime == -1){
			...
			if(ptr.nextptr[nchild].resultTF != null)
			{
				...
				for (int nrow = 0; nrow < hangshu; nrow++) {
					for(int ncol = 0; ncol<5; ncol++){
						tabledata[nrow][ncol] = ptr.nextptr[nchild].resultTF[nrow][ncol];
					}
				}
			}
		}else{
			...
			columnNames[3] = "Num sub-path";
			columnNames[4] = "Enrichment p-value";
			columnNames[5] = theTimeiohmm.theDataSet.dsamplemins[ndepth+ntime]+" DE p-value";
			...
			if(ptr.nextptr[nchild].resultTF!=null)
			{
				...
				for (int nrow = 0; nrow < hangshu; nrow++) {
					...
					tabledata[nrow][5] = ptr.nextptr[nchild].resultTF[nrow][5+ntime];
					tabledata[nrow][6] = ptr.nextptr[nchild].resultTF[nrow][5+ntime+alltime];
				}
			}
		}
