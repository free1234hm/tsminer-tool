...
public class HeatMap extends JPanel
{
     ...
     */
    public HeatMap(double[][] data, String[] dsamplemins, 
    	   String[] genelist, boolean useGraphicsYAxis, Color[] colors)
    {
        super();
        ...
        drawData(); 
        //addMouseListener(new MouseListTest());
    ...
    private void drawData()
    {
        ...
        for (int x = 0; x < data.length; x++)
        {
            for (int y = 0; y < data[0].length; y++)
            {
            	//System.out.println(dataColorIndices[x][y]);
    ...
    class MouseMoved extends MouseAdapter
    {
        public void mouseMoved(MouseEvent e) {
        	...
        	if(getx>30 && getx<(width - 30) && gety>30 && gety<(height - 30)){
        		...
        		int ll = gety - 30;
        		int index = (int) Math.floor(ll/yDist);
        		//System.out.println(getx+"\t"+gety+"\t"+index);
        		if(currentgene != index && index<genelist.length){
        			currentgene = index;
        			setToolTipText(genelist[currentgene]);
        		}
        	}else{
        		currentgene = -1;
        	}
    ...
    public void paintComponent(Graphics g)
    {
        ...
        if(dsamplemins != null && dsamplemins.length>0){
        	// X-Axis ticks
            if (drawXTicks)
            {
