 ...
 */
public class TSMiner_NaiveBayes {
	int numcols;
	int numclasses;
	double[] pClass;
	int noffset;
	 ...
	 */
	double[][][] pNB;
	// nBaseCount is used in many classes outside of TSMiner_NaiveBayes
	 ...
	 */
	public TSMiner_NaiveBayes(int[][] traindata, int[][] traindataIndex,
		...
		for (int nrow = 0; nrow < numrows; nrow++) {
			...
			for (int ncol = 0; ncol < numcols; ncol++) {
				...
				if ((nfeatureindex >= 0) && (nfeatureindex < numfeaturevals)) {
					...
					nBaseCount[ncol][nfeatureindex]++;  //两列，第一列是其不调控的基因数，第二列是其调控的及因数
				}
			}
			pClass[y[nrow]]++;
		}
