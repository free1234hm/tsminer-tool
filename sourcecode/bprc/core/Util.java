...
public class Util
{
   ...
   public static int romanToNumeric(String roman) 
   {
    ...
    public static String[][] BubbleSort_dec(String[][] r, Integer n, Integer col) //升序冒泡排序
 	{
 		 int low = 0;   
 		    int high= n - 1; //设置变量的初始值  
 		    String[] tmp;
    ...
    public static String[][] BubbleSort_inc(String[][] r, Integer n, Integer col) //升序冒泡排序
 	{
 		 int low = 0;   
 		    int high= n - 1; //设置变量的初始值  
 		    String[] tmp;
    ...
    public static double getEuclideanDis(double[] p1, double[] p2) {
        double count_dis = 0;
        ...
        for (int i = 0; i < p1.length; i++) {
            count_dis += Math.pow(p1[i] - p2[i], 2);
        }
        return Math.sqrt(count_dis);
    }
    
    public static double[][] Hierarchical(double[][] r)
   	{
    	double[][] result = new double[r.length][r[0].length];
    	...
    	while(list.size()>0){
        	...
        	for(int i:list){
        		double dis = getEuclideanDis(r[current], r[i]);
        		if(dis<dismin){
        			dismin=dis;
        			indexmin = i;
        		}
        	...
        	current = indexmin;
        	resultlist.add(indexmin);
        	Iterator<Integer> chk_it = list.iterator();  
        	while(chk_it.hasNext()){  
        	    int checkWork = chk_it.next();  
        	    if(checkWork == indexmin){  
        	        chk_it.remove();  
        	    }  
        	}
    	}
