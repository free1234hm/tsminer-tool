...
public class DataSetCore {
	 ...
	 * dmaxval
	 */
	private DataSetCore filtergenes() {
		...
		for (int nrow = 0; nrow < numrows; nrow++) {			
			...
			for (int ncol = 1; ncol < numcols; ncol++) {
				...
				if (bfcto0 && (pmavalues[nrow][ncol] > 0) && (Math.abs(data[nrow][ncol]) > dmaxfc20)) {
					dmaxfc20 = Math.abs(data[nrow][ncol]);
				}
				if (bfctopre && (pmavalues[nrow][ncol] > 0) && (pmavalues[nrow][ncol-1] > 0) && (Math.abs(data[nrow][ncol]-data[nrow][ncol-1]) > dmaxfc2pre)) {
					dmaxfc2pre = Math.abs(data[nrow][ncol]-data[nrow][ncol-1]);
				}
			}

			if ((bmaxminval && dmax - dmin >= dthresholdvalue) || 
				(bfcto0 && dmaxfc20 >= dthresholdvalue) || 
				(bfctopre && dmaxfc2pre >= dthresholdvalue)) {
