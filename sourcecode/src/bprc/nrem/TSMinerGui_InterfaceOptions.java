 ...
 */
public class TSMinerGui_InterfaceOptions extends JPanel implements ChangeListener,
	 ...
	 */
	public TSMinerGui_InterfaceOptions(JFrame theFrame, TSMinerGui theTSMinerGui) {
		...
		int npoints = theTSMinerGui.theTimeiohmm.theDataSet.data[0].length - 1;
		theColorSlider = new JSlider(0, npoints, theTSMinerGui.ncolortime);
		...
		theSlider = new JSlider(0, 100, (int) (theTSMinerGui.dscaley * 10));
		theDictionary = new Hashtable();
		...
		pvalLabel = new JLabel("Y-axis scale factor is "
				+ doubleToSz(theTSMinerGui.dscaley));
		...
		theSliderX = new JSlider(0, 100, (int) (theTSMinerGui.dscalex * 10));
		...
		pvalLabelX = new JLabel("X-axis scale factor is "
				+ doubleToSz(theTSMinerGui.dscalex));
		...
		nodeLabelPanel.setBackground(new Color((float) 0.0, (float) 1.0,
				(float) 0.0, (float) 0.4));
		add(nodeLabelPanel);
		theNodeSlider = new JSlider(0, 50, (int) (theTSMinerGui.dnodek * 10));
		...
		nodeLabel = new JLabel("Node area scale factor is "
				+ doubleToSz(theTSMinerGui.dnodek));
		nodelabelPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
		...
		attachBox.setSelected(theTSMinerGui.battachlabels);
	...
	public void actionPerformed(ActionEvent e) {
		theTSMinerGui.battachlabels = attachBox.isSelected();
		...
		if ((theTSMinerGui.battachlabels) && (!theTSMinerGui.bglobalnode)) {
			theTSMinerGui.hidelabels();
		} else if (!theTSMinerGui.battachlabels) {
			theTSMinerGui.showlabels();
		}
	}
	...
	public void itemStateChanged(ItemEvent e) {
		if (realButton.isSelected()) {
			theTSMinerGui.brealXaxis = true;
		} else {
			theTSMinerGui.brealXaxis = false;
		}

		if (holdButton.isSelected()) {
			theTSMinerGui.bholdedge = true;
		} else {
			theTSMinerGui.bholdedge = false;
		}

		theTSMinerGui.drawmain();
	}
