 ...
 */
public final class TSMiner_FastLogistic2 {
	 ...
	 * Empty constructor
	 */
	public TSMiner_FastLogistic2() {
	}

	/**
	 * Constructor where the coefficients are given to the classifier
	 */
	public TSMiner_FastLogistic2(int[][] traindataindex, double[][] traindata,
			int[][] traindataTFindex, double[][] traindataTF, int[] y,
			int numclasses, double[] dcoeff, int numbits, int[] regTypes) { //读已有树结构时才运行的方法
	...
	public TSMiner_FastLogistic2(int[][] traindataindex, double[][] traindata,
		...
		if (numclasses == 2) {
			dipcache = new double[dtrainweight.length];  //基因数*节点数
		} else {
	 ...
	 */
	public Object clone() {
		TSMiner_FastLogistic2 fl = new TSMiner_FastLogistic2();
	...
	public void reinit(int[][] traindataindex, double[][] traindata,
		...
		if (this.numclasses != numclasses) {  //如果此节点的子节点数不等于输入子节点数，及该节点添加了分岔
			this.numclasses = numclasses;
				...
				if (nelmod == 1) {
					...
					for (int ntfindex = 0; ntfindex < traindataTFindex[0].length; ntfindex++) {
						try{
							...
							System.out.println(g.length+"\t"+nel);
							System.out.println(traindataTFindex[0].length+"\t"+ntfindex);
							System.out.println(traindataTF[0].length+"\t"+ntfindex);
							System.out.println(dyerror.length+"\t"+traindataTFindex[0][ntfindex]);
							e.printStackTrace(); 
						}
					...
					for (int ntfindex = 0; ntfindex < traindataTFindex[nelmod1].length; ntfindex++) {					
						try{
							g[nel] += dyerror[traindataTFindex[nelmod1][ntfindex]] * traindataTF[nelmod1][ntfindex];
						}catch (Exception e) {
							System.out.println(g.length+"\t"+nel);
							System.out.println(traindataTFindex.length+"\t"+nelmod1);
							System.out.println(traindataTFindex[nelmod1].length+"\t"+ntfindex);
							System.out.println(traindataTF.length+"\t"+nelmod1);
							System.out.println(traindataTF[nelmod1].length+"\t"+ntfindex);
							System.out.println(dyerror.length+"\t"+traindataTFindex[nelmod1][ntfindex]);
							e.printStackTrace(); 
						}
					}
				}
			...
			if (nel == 0) {
				...
				if (bfirsttime) {
					...
					for (int nrow = 0; nrow < traindata.length; nrow++) {
						...
						double[] dx = traindata[nrow];//相互作用值数组
						int[] dxIndex = traindataindex[nrow]; //tf编号数组
						int nxindex;
						...
						if (y[nrow] == 0) {
							...
							dyerror[nrow] = dtrainweight[nrow] * (1 - dpbinval); //第一个子节点为正值
							
						} else {
							// When the jth training example belongs to class 1,
							// y'_j = 0 ???
							// trainweight * (y'_j - p_j(w))
							dyerror[nrow] = -dtrainweight[nrow] * dpbinval; //第二个子节点为负值
							
						}
						// Sum over all training data
						g[0] += dyerror[nrow];  //所有基因经过第一个子节点的概率和-经过第二个子节点概率和，正则第一个节点概率高，负则第二个节点概率高
					}
					
				} else // bfirsttime is false 不是第一次运行
				{
				...
				if (nel == 1) {
					...
					for (int ntfindex = 0; ntfindex < traindataTFindex[nel1].length; ntfindex++) {				
						try{
							...
							System.out.println(g.length+"\t"+nel);
							System.out.println(traindataTFindex.length+"\t"+nel1);
							System.out.println(traindataTFindex[nel1].length+"\t"+ntfindex);
							System.out.println(traindataTF.length+"\t"+nel1);
							System.out.println(traindataTF[nel1].length+"\t"+ntfindex);
							System.out.println(dyerror.length+"\t"+traindataTFindex[nel1][ntfindex]);
							e.printStackTrace(); 
						}
					}
				}
...
//					+ dcoeff[nel]);
	...
	private void computeB() {
		...
		//System.out.println(numclasses+";"+traindataindex.length+";"+dtrainweight.length);
		for (int nrow = 0; nrow < traindataindex.length; nrow++) {
			B[0] += dtrainweight[nrow]; //B[0]为每个基因经过每个子节点的总概率

			// trainweight * x_j * (x_j)^T
			for (int nrowindex = 0; nrowindex < traindataindex[nrow].length; nrowindex++) {
				
				B[traindataindex[nrow][nrowindex] + 1] += traindata[nrow][nrowindex] * traindata[nrow][nrowindex] * dtrainweight[nrow];
				
			}
		}
		/*
		try{
			outXml = new BufferedWriter(new FileWriter("D:\\B.txt"));
		...
		*/
       //B矩阵长度为TF数+1，第一个元素为所有基因经过所有子节点的总概率，之后每个元素代表被这个转录因子调控的基因经过所有子节点的总概率
		// Every element on the diagonal of the matrix formed by:
		// -1/2 * [I - 1*(1)^T / m]
		// has value dmI. Based on the first part of Equation 8 in Krishnapuram
		// et al.
		double dmI = -0.5 + 0.5 / numclasses;//如果只有一个子节点，那么B所有元素为0
		//System.out.println(numclasses+";"+dmI);
	...
	public void trainbin() {
		...
		for (int nrow = 0; nrow < numrows; nrow++) {
			...
			for (int ngeneindex = 0; ngeneindex < traindataindex[nrow].length; ngeneindex++) {
				int ntf = traindataindex[nrow][ngeneindex];  //ntf为调控对应基因的tf
				dip += traindata[nrow][ngeneindex] * dcoeff[ntf + 1];
			}
			//System.out.println(dip);
			double dmu = 1 / (1 + Math.exp(-dip)); //此基因经过节点0的概率
			if (y[nrow] == 1) {
				dolddev += dtrainweight[nrow] * Math.log(1 - dmu);//y=0代表第一个子节点，y=1代表第二个子节点
			} else {
				dolddev += dtrainweight[nrow] * Math.log(dmu);
			}
		}

		int niteration = 0;  //重复计数
		double dratio;

		boolean bfirst = true;  //是第一次训练吗

		do {//开始训练
			boolean bnew = true;
			for (int nel = 0; nel < numcols; nel++) {//numcols为tf数+1
