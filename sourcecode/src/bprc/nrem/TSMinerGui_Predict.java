 ...
 */
public class TSMinerGui_Predict extends JPanel implements ActionListener {
	final static Color bgColor = Color.white;
	final static Color fgColor = Color.black;
	TSMiner_Timeiohmm.Treenode rootptr;
	JRadioButton intersectButton, unionButton;
	int[] tfVals;
	TSMinerGui_PredictModel hmst;
	TableSorter sorter;
	 ...
	 */
	public TSMinerGui_Predict(JFrame theFrame, TSMinerGui theTSMinerGui,
			final TSMiner_Timeiohmm.Treenode rootptr) {
		this.theFrame = theFrame;
		this.theTSMinerGui = theTSMinerGui;
		this.theTimeiohmm = theTSMinerGui.theTimeiohmm;
		...
		hmst = new TSMinerGui_PredictModel(tabledata, columnNames);
	 ...
	 * on bvisible
	 */
	void setPredictTextVisible(TSMiner_Timeiohmm.Treenode treeptr, boolean bvisible) {
		if (treeptr != null) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (szcommand.equals("show")) {
			...
			theTSMinerGui.bshowpredict = true;
			for (int nrow = 0; nrow < numrows; nrow++) {
				for (int ncol = 1; ncol < numcols; ncol++) {
			...
			theTSMinerGui.bshowpredict = false;
			setPredictTextVisible(rootptr, false);
			hideButton.setEnabled(false);
		} else if (szcommand.equals("help")) {
			String szMessage = "TSMiner allows one to view for any set of transcription factor-gene regulation interaction inputs, "
