...
import bprc.core.*;
import bprc.nrem.TSMiner_Timeiohmm.SigTFRecv2;
 ...
 * Result GUI
 */
public class TSMinerGui extends PFrame implements ComponentListener {
	...
	Treenode selectednode;// array list of nodes
	// and edges whose
	 ...
	 * Class constructor
	 */
	public TSMinerGui(TSMiner_Timeiohmm theTimeiohmm, 
			TSMiner_Timeiohmm.Treenode treecopy, boolean brealXaxisDEF,
			double dYaxisDEF, double dXaxisDEF, int nKeyInputTypeDEF,
			double dKeyInputXDEF, double dpercentDEF, String szFinal,
			double dnodekDEF, int npermutationval) {
		super("TSMiner - Main Interface " + szFinal, brealXaxisDEF, canvas);
	...
	public void drawmain() {
		synchronized (datasetlock) {
			...
			final TSMinerGui theTSMinerGui = this;
			
			activeTFButton = PPath.createRectangle((float) 0.0, (float) 0.0,
					(float) BUTTONRECWIDTH, (float) 18.0);
			PText activeTFText = new PText("TF Summary");
			activeTFText.setFont(new Font("times", Font.PLAIN, 12));
			activeTFText.translate(26, 2);
			activeTFButton.setPaint(buttonColor);
			activeTFButton.addChild(activeTFText);
			activeTFButton.addInputEventListener(new PBasicInputEventHandler() {
										...
										for(int i=0;i<theTimeiohmm.theDataSet.dsamplemins.length;i++){
									    	...
									    	if(tfs != null && tfs.size()>0 && key.equals(theTimeiohmm.theDataSet.dsamplemins[i])){
									    		...
									    		TSMinerGui_TFTable newContentPane1 = new TSMinerGui_TFTable(
														theTSMinerGui, treecopy, i, frame,
														theTimeiohmm, table1, table2, (String)key, npermutationval);
						...
						public void mousePressed(PInputEvent event) {
							javax.swing.SwingUtilities.invokeLater(new Runnable() {
										public void run() {
											...
											TSMinerGui_GeneTable newContentPane = new TSMinerGui_GeneTable(
													frame,
			...
			final TSMinerGui ftheTSMinerGui = this;
					...
					javax.swing.SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							if (yscaleFrame == null) {
								...
								theYScalegui = new TSMinerGui_InterfaceOptions(
										yscaleFrame, ftheTSMinerGui);
					...
					javax.swing.SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							if (predictFrame == null) {
								...
								TSMinerGui_Predict newContentPane = new TSMinerGui_Predict(
										predictFrame, ftheTSMinerGui,
										ftheTSMinerGui.treecopy);
			...
			saveTSMinerButton = PPath.createRectangle((float) 0.0, (float) 0.0,
					(float) BUTTONRECWIDTH, (float) 18.0);

			PText saveTSMinerText;
			saveTSMinerText = new PText(" Save Image ");
			saveTSMinerText.setFont(new Font("times", Font.PLAIN, 12));
			saveTSMinerText.translate(18, 2);
			saveTSMinerButton.setPaint(buttonColor);
			saveTSMinerButton.addChild(saveTSMinerText);

			saveTSMinerButton.addInputEventListener(new PBasicInputEventHandler() {
				public void mousePressed(PInputEvent event) {
					javax.swing.SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							if (saveTSMinerFrame == null) {
								saveTSMinerFrame = new JFrame("Save as Image");
								saveTSMinerFrame
										.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
								saveTSMinerFrame.setLocation(400, 300);
								TSMinerGui_SaveTSMiner newContentPane = new TSMinerGui_SaveTSMiner(
										ftheTSMinerGui, saveTSMinerFrame);
								newContentPane.setOpaque(true);
								// content panes must be opaque
								saveTSMinerFrame.setContentPane(newContentPane);
								// Display the window.
								saveTSMinerFrame.pack();
							} else {
								saveTSMinerFrame.setExtendedState(Frame.NORMAL);
							}
							saveTSMinerFrame.setVisible(true);
						}
					});
				}
			});
			...
			saveButton.addInputEventListener(new PBasicInputEventHandler() {
				public void mousePressed(PInputEvent event) {
					try {
						...
						// Fixes a bug in which the TSMiner model is dragged automatically
				...
				public void mousePressed(PInputEvent event) {
					javax.swing.SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							...
							String szMessage = "This is the display interface window of TSMiner. "
									...
									+ "on an edge brings up info about the TFs regulating genes assigned to the path.\n\n"
									+ "The buttons along the bottom function as follows: \n"
									+ "TF Summary - displays the activated/repressed TFs for different time points\n"
			...
			theCamera.addChild(activeTFButton);
			...
			activeTFButton.translate(INITLEFT + nspacing, nheight - 77);
			genetableButton.translate(INITLEFT +  2*nspacing, nheight - 77);
			hideButton.translate(INITLEFT + 3 * nspacing, nheight - 77);
			timeButton.translate(INITLEFT + 4 * nspacing, nheight - 77);
			saveModelButton.translate(INITLEFT + 5 * nspacing, nheight - 77);
			helpButton.translate(INITLEFT + 6 * nspacing, nheight - 77);
			
			//theCamera.addChild(saveTSMinerButton);
			...
			/*
			saveTSMinerButton.translate(INITLEFT + 4 * nspacing, nheight - 77);
			...
			saveButton.translate(INITLEFT + 4 * nspacing, nheight - 77);
			*/
					...
					javax.swing.SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							if (keyinputFrame == null) {
								...
								TSMinerGui_KeyInputs newContentPane = new TSMinerGui_KeyInputs(
										keyinputFrame, ftheTSMinerGui);
								newContentPane.setOpaque(true);
								// content panes must be opaque
...
/*
		...
		for (int nindex = 0; nindex < nsizesig; nindex++) {
			TSMinerGui.SigInfoRec theSigInfoRec = hidesigList.get(nindex);
...
*/
	 ...
	 */
	public void setSigText(TreeSet<TSMiner_Timeiohmm.SigTFRecv2> tsSigTF,
			PPath boundingRect, int ntype, int ndepth, boolean displayTF,
			boolean displayMIRNA, TSMiner_Timeiohmm.Treenode ptr, boolean color) {
		...
		if (ntype == nKeyInputType) {
			...
			for (Object theTFRecObj : tsSigTF) {
				TSMiner_Timeiohmm.SigTFRecv2 theTFRec = (TSMiner_Timeiohmm.SigTFRecv2) theTFRecObj;
				...
				case RegulatorBindingData.MIRNA:
					lookup = theTimeiohmm.reg2DataSetIndex.get(theTFRec.szname
							.toUpperCase());
					if (lookup != null) {
						double expression = theTimeiohmm.miRNADataSet.data[lookup][ndepth];
	...
	public static class SigInfoRec {
		TreeSet<TSMiner_Timeiohmm.SigTFRecv2> theSigTF;
		int ntype;
		PNode border;
		...
		SigInfoRec(PNode border, TreeSet<TSMiner_Timeiohmm.SigTFRecv2> tsSigTF,
	...
	static class SigTFRecv2Comp implements
			Comparator<TSMiner_Timeiohmm.SigTFRecv2> {
	 ...
	 */
	public PBasicInputEventHandler drawNodes(TSMiner_Timeiohmm.Treenode ptr,
		...
		if (ptr != null) {
			...
			if (ndepth == 0) {
				...
				final TSMiner_Timeiohmm.Treenode fptr = ptr;
				final TSMinerGui theTSMinerGui = this;
						...
						if (event.getButton() == MouseEvent.BUTTON3) {
							javax.swing.SwingUtilities.invokeLater(new Runnable() {
										public void run() {
											...
											if(fptr.deTF != null){
												if(fptr.parent.numchildren>1){
													int colnum = fptr.resultTF[0].length;
														...
														for(int i=0;i<time;i++){
															...
															TSMinerGui_EdgeTable newContentPane1 = new TSMinerGui_EdgeTable(
																	theTSMinerGui, frame, frame1,
																	theTimeiohmm, fptr.parent,
																	ncurrscore, ndepth,
																	fnchild, nKeyInputType,
																	i, time, false, npermutationval);
															newContentPane1.setOpaque(true); // content panes must be opaque
															tabbedPane.addTab("TFs functioning at "+segment, null, 
																	newContentPane1, "TFs functioning at "+segment);
														}
													...
													TSMinerGui_EdgeTable2 newContentPane1 = new TSMinerGui_EdgeTable2(
															...
															theTimeiohmm, fptr.parent,
															ncurrscore, ndepth,
															fnchild, nKeyInputType, false, npermutationval);
													newContentPane1.setOpaque(true); // content panes must be opaque
													tabbedPane.addTab("TFs functioning at "+segment, null, 
															newContentPane1, "TFs functioning at "+segment);
												}
...
//							- ddiameter / 2);
						...
						if (badd) {
							...
							while (els.hasMoreElements()) {
								...
								if (bcircle) {
									...
									nval = nval.pow((data[0].length - theCircleID.nminparentlevel));
								} else {
									// nval = (int)
									// Math.pow(theTimeiohmm.nmaxchild,data[0].length-theCircleID.nprevminparentlevel);
									nval = nval.pow((data[0].length - theCircleID.nprevminparentlevel));
								}
	...
	public void printDefaults(PrintWriter pwsave) {
		...
		pwsave.println("Minimum_Standard_Deviation\t"
		...
		pwsave.println("Number_of_samples_for_randomized_multiple_hypothesis_correction"
		...
		if (theTimeiohmm.miRNAInteractionDataFile != null
			...
			pwsave.println("#TSMiner with miRNA Options");
		...
		{
			pwsave.println("#STSMiner Options");
		...
		pwsave.print("Key_Input_Significance_Based_On[Path Significance Conditional on Split,Path Significance Overall,Split Significance]\t");
		if (nKeyInputType == 0) {
	...
	public void batchSave(String filename) {
		...
		// The content pane shoul be a TSMinerGUI_SaveModel
		((TSMinerGui_SaveModel) c).batchSave(filename);
	}
