 ...
 */
public class TSMinerGui_EdgeTable extends JPanel implements ActionListener {
	/**
	 * 
	 */
	 ...
	 */
	public TSMinerGui_EdgeTable(TSMinerGui theTSMinerGui, JDialog dialog, JFrame frame1,
			TSMiner_Timeiohmm theTimeiohmm, TSMiner_Timeiohmm.Treenode ptr,
			BigInteger npathID, int ndepth, int nchild, int ntype, 
			int ntime, int alltime, boolean broot, int npermutationval) {
		// assuming that if called with root node then only one child so stats
		// for root will be same as first child
		this.theTSMinerGui = theTSMinerGui;
		this.dialog = dialog;
		...
		tabledata = new String[numrows][7];
		columnNames[0] = "TF";
		columnNames[1] = "Num total";
		columnNames[2] = "Num parent";
		columnNames[3] = "Num sub-path";
		columnNames[4] = "Enrichment q-value";
		columnNames[5] = theTimeiohmm.theDataSet.dsamplemins[ndepth+ntime]+" DE q-value";
		columnNames[6] = theTimeiohmm.theDataSet.dsamplemins[ndepth+ntime]+" Avg. fold-change";
		int nrowindex = 0;
		if(ptr.nextptr[nchild].resultTF!=null)
		{
			int hangshu = ptr.nextptr[nchild].resultTF.length;
			for (int nrow = 0; nrow < hangshu; nrow++) {
				for(int ncol = 0; ncol<5; ncol++){
					tabledata[nrowindex][ncol] = ptr.nextptr[nchild].resultTF[nrow][ncol];
					}
				tabledata[nrowindex][5] = ptr.nextptr[nchild].resultTF[nrow][5+ntime];
				tabledata[nrowindex][6] = ptr.nextptr[nchild].resultTF[nrow][5+ntime+alltime];
				nrowindex++;
				}
			}
	...
	private void addBottom() {
		...
		Integer colorID = (Integer) theTSMinerGui.htLineIDtoColorID.get(ndepth+ ";" + npathID);
		Color initcolor = (Color) theTSMinerGui.htColorIDtoColor.get(colorID);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (szCommand.equals("pathway")) {
			String selecteditem = theTimeiohmm.theDataSet.dsamplemins[ndepth+ntime];
			...
			if(table.getSelectedRows()==null || table.getSelectedRows().length==0){
				JOptionPane.showMessageDialog(theframe, 
					...
					if (theTimeiohmm.raw_gobinding == null || 
						theTimeiohmm.raw_gobinding.reg2GeneBindingIndex[ndepth+ntime] == null) {
						throw new IllegalArgumentException(     
								"No gene annotation input file given!");
					}
					
					HashMap<String,Integer> TFindex = theTimeiohmm.raw_reg2DataSetIndex; //用rawDataSet
					int[][] Pathway2Geneindex = theTimeiohmm.raw_gobinding.reg2GeneBindingIndex[ndepth+ntime];//用rawDataSet
					...
					javax.swing.SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							...
							if(Resulttable!=null &&Resulttable.length>0){
								String name = theTimeiohmm.theDataSet.dsamplemins[ndepth+ntime];
								TSMinerGui_PathwayTable newContentPane1 = new TSMinerGui_PathwayTable(
										Resulttable, frame, pathwayframe,
										pathwaygene,tf_tg2,tf_pg2, ndepth+ntime, selecteditem, rawDataSet, 0);
								newContentPane1.setOpaque(true); // content panes must be opaque
								tabbedPane.addTab(name+" Pathway", null, newContentPane1,name+" Pathway");
			...
			Integer colorID = (Integer) theTSMinerGui.htLineIDtoColorID.get(ndepth+ntime
					+ ";" + npathID);
			Color initcolor = (Color) theTSMinerGui.htColorIDtoColor.get(colorID);
			...
			if (newColor != null) {
				...
				ArrayList linesToUpdate = (ArrayList) theTSMinerGui.htColorIDtoLinesList
						.get(colorID);
				int nsize = linesToUpdate.size();
				theTSMinerGui.htColorIDtoColor.put(colorID, newColor);
				for (int nindex = 0; nindex < nsize; nindex++) {
					PPath currPath = (PPath) linesToUpdate.get(nindex);
					if ((theTSMinerGui.theSelectedRec.selectedNode != currPath)
							|| (theTSMinerGui.theSelectedRec.bcircle)) {
						currPath.setStrokePaint(newColor);
					} else {
						theTSMinerGui.htColors.put(
								theTSMinerGui.theSelectedRec.selectedNode,
								newColor);
					}
				}

				ArrayList circlesToUpdate = (ArrayList) theTSMinerGui.htColorIDtoCircleList
						.get(colorID);
				if (circlesToUpdate != null) {
					nsize = circlesToUpdate.size();
					for (int nindex = 0; nindex < nsize; nindex++) {
						PNode currCircle = (PNode) circlesToUpdate.get(nindex);
						if ((theTSMinerGui.theSelectedRec.selectedNode != currCircle)
								|| (!theTSMinerGui.theSelectedRec.bcircle)) {
