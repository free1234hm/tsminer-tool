 ...
 */
public class TSMinerGui_KeyInputs extends JPanel implements ActionListener,
	 ...
	 */
	public TSMinerGui_KeyInputs(JFrame theFrametemp, TSMinerGui theTSMinerGuitemp) {
		this.theFrame = theFrametemp;
		this.theTSMinerGui = theTSMinerGuitemp;
		nf2 = NumberFormat.getInstance(Locale.ENGLISH);
		nf2.setMinimumFractionDigits(2);
		nf2.setMaximumFractionDigits(2);
		boolean mirnapresent = theTSMinerGui.theTimeiohmm.bindingData.existingRegTypes
		...
		int ninitval = (int) Math.round(-100
				* Math.log(theTSMinerGui.dkeyinputpvalue) / Math.log(10)) / 10;

		// pval = 10^-X
		// log p-val/log 10 = -X
		// X = -log p-val/log 10
		theTSMinerGui.dkeyinputpvalue = Math.pow(10, -ninitval / 10.0);
		theTSMinerGui.dkeymiRNAinputpvalue = Math.pow(10, -ninitval / 10.0);
		...
		theTSMinerGui.blowbelow = true;
		theTSMinerGui.blowabove = true;
		...
		if (theTSMinerGui.nKeyInputType == 0) {
			splitButton.setSelected(true);
		} else if (theTSMinerGui.nKeyInputType == 1) {
			edgeSplitButton.setSelected(true);
		} else if (theTSMinerGui.nKeyInputType == 778) {
			firstActivityButton.setSelected(true);
		} else if (theTSMinerGui.nKeyInputType == 777) {
			activityButton.setSelected(true);
		} else // (theTSMinerGui.nKeyInputType == 2)
		{
		...
		if(theTSMinerGui.theTimeiohmm.bindingData.regPriors != null)
		{
		...
		if (TSMiner_Timeiohmm.BDEBUG) {
			System.out.println("!!!!" + theTSMinerGui.dsplitpercent);
		}
		int ninitvalpercent = (int) Math.round(theTSMinerGui.dsplitpercent * 100);

		// pval = 10^-X
		// log p-val/log 10 = -X
		// X = -log p-val/log 10
		theTSMinerGui.dsplitpercent = ninitvalpercent / 100.0;
		...
		percentLabel = new JLabel(" Minimum Split % is "
				+ nf2.format(100 * theTSMinerGui.dsplitpercent));
		...
		regToggleButton.setForeground(theTSMinerGui.keyInputLabelColor);
		...
		regColorToggleButton.setForeground(theTSMinerGui.keyInputLabelColor);
				...
				javax.swing.SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						if (saveRegsFrame == null) {
							...
							TSMinerGui_SaveSigRegs newContentPane = new TSMinerGui_SaveSigRegs(
									theTSMinerGui, saveRegsFrame);
	...
	public void stateChanged(ChangeEvent e) {
		...
		theTSMinerGui.sigRegs = new HashSet<String>();
		if (!source.getValueIsAdjusting()) {
			if (source == theSlider) {
				theTSMinerGui.dkeyinputpvalue = Math.pow(10,
						-source.getValue() / 10.0);
				pvalLabel.setText("X = " + source.getValue() / 10.0
						+ "; score threshold is "
						+ doubleToSz(theTSMinerGui.dkeyinputpvalue));
				int nsize = theTSMinerGui.hidesigList.size();
				for (int nindex = 0; nindex < nsize; nindex++) {
					TSMinerGui.SigInfoRec theSigInfoRec = (TSMinerGui.SigInfoRec) theTSMinerGui.hidesigList
							.get(nindex);
					theSigInfoRec.border.removeAllChildren();
					theTSMinerGui.setSigText(theSigInfoRec.theSigTF,
							(PPath) theSigInfoRec.border, theSigInfoRec.ntype,
							theSigInfoRec.ndepth, displayTF, displayMIRNA,
							theSigInfoRec.node, theTSMinerGui.colorSigTFs);
				}
			} else if (source == theSliderPercent) {
				theTSMinerGui.dsplitpercent = source.getValue() / 100.0;
				...
				for (int nindex = 0; nindex < nsize; nindex++) {
					...
					if (theSigInfoRec.ntype == 1) // split path type
					{
						...
						theTSMinerGui.setSigText(theSigInfoRec.theSigTF,
								...
								displayTF, displayMIRNA, theSigInfoRec.node, theTSMinerGui.colorSigTFs);
					}
				}
			} else if (source == miRNASlider) {
				theTSMinerGui.dkeymiRNAinputpvalue = Math.pow(10, -source
						.getValue() / 10.0);
				pvalmiRNALabel.setText("X = " + source.getValue() / 10.0
						+ "; score threshold is "
						+ doubleToSz(theTSMinerGui.dkeymiRNAinputpvalue));
				int nsize = theTSMinerGui.hidesigList.size();
				for (int nindex = 0; nindex < nsize; nindex++) {
					TSMinerGui.SigInfoRec theSigInfoRec = (TSMinerGui.SigInfoRec) theTSMinerGui.hidesigList
							.get(nindex);
	...
	public void itemStateChanged(ItemEvent e) {
		if (splitButton.isSelected()) {
			theTSMinerGui.nKeyInputType = 0;
		} else if (edgeSplitButton.isSelected()) {
			theTSMinerGui.nKeyInputType = 1;
		} else if (edgeFullButton.isSelected()) {
			theTSMinerGui.nKeyInputType = 2;
		} else if (activityButton.isSelected()) {
			theTSMinerGui.nKeyInputType = 777;
		} else if (firstActivityButton.isSelected()) {
			theTSMinerGui.nKeyInputType = 778;
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (szcommand.equals("help")) {
			String szMessage = "This window controls the Key Input labels appearing on the TSMiner output interface.   "
			...
			theTSMinerGui.colorSigTFs = !theTSMinerGui.colorSigTFs;
			...
			if (theTSMinerGui.bshowkeyinputs) {
				hideButton.setText("Show Key Reg Labels");
				theTSMinerGui.bshowkeyinputs = false;
				...
				theTSMinerGui.bshowkeyinputs = true;
				for (int nindex = 0; nindex < nsize; nindex++) {
					TSMinerGui.SigInfoRec theSigInfoRec = (TSMinerGui.SigInfoRec) theTSMinerGui.hidesigList
							.get(nindex);
					PPath rect = (PPath) theSigInfoRec.border;
